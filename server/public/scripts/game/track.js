  ...
  exports.Track.prototype.loadWithConfig = function(config, callback) {
    ...
    if (config.terrain) {
      ...
      source.load(terrainConfig, function() {
        ...
        function drawCircle(map, img, numChan, chan, x, y, value, radius, hardness, opacity) {
          ...
          for (iY = mMinY; iY <= mMaxY; ++iY) {
            ...
            for (iX = mMinX; iX <= mMaxX; ++iX) {
              ...
              if (wR < radius) {
                ...
                i = (wrap(iX, map.cx) + map.cx * wrap(iY, map.cy)) * numChan + chan;
                img[i] += (value - img[i]) * weight * opacity;
              }
            }
          }
        };
        function drawCircleDisplacement(map, x, y, value, radius, hardness, opacity) {
          drawCircle(map, map.displacement, 1, 0, x, y, value / map.scale.z, radius, hardness, opacity);
        }
